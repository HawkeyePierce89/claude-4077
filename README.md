...
- *review* — the project's own test/build gates first, then the branch goes to `revmux:revmux` with the plan's acceptance criteria as the goal and the ticket as context, `comprehensive` profile. revmux presents the findings and asks what to do (that's the second gate): fix and re-review, run its fix loop until clean, or stop. Claude adds the gate results.
